...
@pytest.mark.task3
@pytest.mark.trio
async def test_task3a_integration():
    """Integration test for task3a with mocked HTTP responses."""
    ...
    with patch('requests.get') as mock_requests_get, \
        ...
        with trio.move_on_after(5.5) as cancel_scope:
        # Call task4
            result = await solution_module.task3a()
        if cancel_scope.cancelled_caught: 
            pytest.fail("Test timed out")
        # Assertions
...
async def test_task3a_timing():
    ...
    with trio.move_on_after(4):
        await solution_module.task3a()
    end_time = trio.current_time()
    
    elapsed_time = end_time - start_time
    print(elapsed_time)
    assert elapsed_time <= 4.5, f"Expected task 4 to take less than 4.5 seconds, got {elapsed_time:.2f} seconds"


@pytest.mark.task3
@pytest.mark.trio
async def test_task3b_timing():
    """Test that task5 can be executed under 4 seconds"""
    # This is a basic test to ensure task4 runs under 4 seconds
    with trio.move_on_after(3):
        start_time = trio.current_time()
        await solution_module.task3b()
    end_time = trio.current_time()
    
    elapsed_time = end_time - start_time
    assert elapsed_time <= 2.5, f"Expected task 3b to take less than 2 seconds, got {elapsed_time:.2f} seconds"
